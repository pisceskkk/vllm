    ...
    def add_new_req_to_recv(
        ...
        req.remote = RemoteMeta(
            ...
            block_positions=kv_transfer_params.get(
                ...
                list(range(len(kv_transfer_params["remote_block_ids"]))),
            ),
...
class NixlConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        self._reqs_need_recv: dict[ReqId, tuple[Request, list[int], list[int]]] = {}
    ...
    @staticmethod
    def _get_request_block_positions(block_ids: list[int]) -> list[int]:
    ...
    def _get_unhashed_block_ids_and_positions(
        ...
        assert len(blocks.blocks) == 1, "Only one KV cache group is supported"
        ...
        for block_pos, block in enumerate(blocks.blocks[0]):
            if block.is_null or block.block_hash is not None:
                continue
            local_block_ids.append(block.block_id)
            local_block_positions.append(block_pos)
        ...
        if self.use_mla and tp_ratio < 0:
            ...
            for remote_tp_rank in remote_tp_ranks:
                ...
                if primary_remote_tp_rank is None:
                    primary_remote_tp_rank_by_dcp_rank[remote_dcp_rank] = (
                        remote_tp_rank
                    )
                    deduped_remote_tp_ranks.append(remote_tp_rank)
                else:
                    duplicate_remote_tp_ranks_by_primary[
                        primary_remote_tp_rank
                    ].append(remote_tp_rank)
    ...
    def _read_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        for i, remote_tp_rank in enumerate(read_plan.remote_tp_ranks):
            ...
            if self.use_mla and read_plan.tp_ratio < 0:
                ...
                for duplicate_remote_tp_rank in (
                    read_plan.duplicate_remote_tp_ranks_by_primary.get(
                        remote_tp_rank,
                        [],
                    )
                ):
