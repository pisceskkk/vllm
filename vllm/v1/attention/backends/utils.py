...
def _make_metadata_with_slice(
    ...
    # TODO(qcs): check if we can split cp_kv_recover_idx as following approach
...
def get_cp_local_seq_lens(
    ...
    kv_cache_interleave_size: int = 1,
) -> torch.Tensor:
    ...
    base = (
        seq_lens_tiled
        // kv_cache_interleave_size
        // cp_world_size
        * kv_cache_interleave_size
    )
    remainder = seq_lens_tiled - base * cp_world_size
    remainder = torch.clip(
        remainder - rank_offsets * kv_cache_interleave_size,
        0,
        kv_cache_interleave_size,
    )
